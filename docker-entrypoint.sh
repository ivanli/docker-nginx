...
function environment() {
  ...
  if [[ -z "${NGINX_DOCROOT}" ]]; then echo "Required variable NGINX_DOCROOT not set" && exit 1; fi
...
function monit() {
  {
    ...
    # Log to stdout so that messages appear in container logs
...
function permissions() {
  ...
  find ${NGINX_DOCROOT} ! -perm 755 -type d -exec /usr/bin/env bash -c 'i="$1"; chmod 775  "$i"' _ {} \;
  find ${NGINX_DOCROOT} ! -perm 644 -type f -exec /usr/bin/env bash -c 'i="$1"; chmod 664 "$i"' _ {} \;
...
function dev() {
  local IFS=";"
  for i in /conf/nginx/sites-available/*.vhost; do
    if [[ ! -e "$i" ]]; then continue; fi

    echo "Checking for certificates for ${i}"
    server_name="$(basename ${i} .vhost)"
    ...
    if [[ ! -f /etc/letsencrypt/live/${server_name}/privkey.pem ]] || [[ ! -f /etc/letsencrypt/live/${server_name}/fullchain.pem ]]; then
      echo "Failed to find SSL files. Installing development SSL certificates for ${server_name}..."
    ...
    if [[ ! -f /etc/letsencrypt/live/www.${server_name}/privkey.pem ]] || [[ ! -f /etc/letsencrypt/live/www.${server_name}/fullchain.pem ]]; then
      echo "Failed to find SSL files. Installing development SSL certificates for www.${server_name}..."
    ...
    # No need to do this as it's done above when .vhosts are created first time.
